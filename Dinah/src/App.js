...
function App() {
  ...
  //       setIsLoading[true]
  //       const { data: facts, error} = await appwrite.
  //   .from("facts")
  //   .select("*")
...
function Header({ showForm, setShowForm }) {
  const appTitle = "Dinah's Workspace";
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  function handleSubmit(e) {
    ...
    // 2. check if data is valid, id=f so, create a new fact
...
function FactList({ facts }) {
  return (
    <section>
      ...
      <p>There are {facts.length} facts the the database. add your own</p>
    </section>
  );
}
